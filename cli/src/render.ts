 ...
 * as one function. Lets a bulb render another bulb inside itself (typebulb.com already runs
...
// Pure bulb-source helpers, re-exported on the browser entry so a host rendering
...
export async function renderBulb(source: string, opts: { theme?: 'light' | 'dark' } = {}): Promise<RenderBulbResult> {
  ...
  // browser ruleset (an inline bulb is client-only). It catches the import-map / sandbox patterns that compile
  // fine but break in an inline bulb (dynamic import, URL/version imports, navigation), turning a silent runtime
  // failure into a precise, fix-named error. Surfaced as a render error so it rides the inline bulb's existing
  // compile-error → `typebulb logs <agent>` readback path (TB-Agent-Mirror-Inline.md, Iteration).
  //
  // We deliberately do NOT pass `dependencies` here, so the UNDECLARED_IMPORT rule stays dormant for an
  // inline bulb: a model-emitted inline bulb often imports a package its config.json doesn't declare, and the
  // import-driven resolver CDN-resolves "latest" and renders fine. An inline bulb is throwaway and forgiving
...
export interface BulbFrameOptions {
  ...
  /** Ceiling for auto-height. The protocol channel is untrusted (the inline bulb's user
   *  code shares the frame's window with the protocol script), so a buggy/hostile
   *  inline bulb could report an absurd height; this bounds the blast radius. Default 100000. */
  maxHeight?: number
  /** Called with a message when the inline bulb throws at runtime (post-mount). */
  onError?: (message: string) => void
  /** Called once, on the inline bulb's first auto-height report — proof the bulb's code ran to
   ...
   *  page — fine for a long-lived host, a leak for one that churns inline bulbs. */
  signal?: AbortSignal
}
 ...
 * included counterpart to renderBulb for hosts that render bulbs inline.
 *
 * Owns the two things an host can't safely or fully do itself: the sandbox
 * policy (see the setAttribute below) and the host↔inline bulb protocol — auto-height
...
export async function createBulbFrame(source: string, opts: BulbFrameOptions = {}): Promise<HTMLIFrameElement> {
  ...
  // the inline bulb script into this page's DOM, storage, and same-origin endpoints —
  ...
  // inline bulb's user code shares this window with the protocol script and can spoof
