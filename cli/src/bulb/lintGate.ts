 ...
 * failure looks like" so the run path and the embed renderer can't drift (and the
 * `TYPE (line N)` shape the embed lint tests pin stays single-sourced).
 */
 ...
 * sandboxed embed (e.g. NAVIGATION), and those patterns are legitimately fine on a local
 * top-level page — `check` and the embed renderer run the full ruleset, the local run does not.
