...
/**
 * Serve the Claude agent viewer (Specs/Typebulb-CLI-Agent-Viewer.md).
 *
 ...
 * viewer is privileged). Runs in the foreground and self-registers like `runWeb`, so
...
export async function runAgentViewer(args: CliArgs): Promise<void> {
  ...
  // Tee console to `<pid>.log` from the outset so `typebulb logs` can tail the viewer
  ...
  // (Specs/Typebulb-CLI-Env.md), so the import must be dynamic and come after loadEnv.
  ...
  reportEnv(envResult, path.join(basePath, 'viewer'))

  // The viewer's built assets live beside this bundle in `dist/agents/claude/`
  ...
  const server = await startServer({
    ...
    port,
    reloadEmitter,
    // The viewer's server.ts exports are the RPC surface (info/poll/listSessions/…).
    getServerExports: () => agentServer as unknown as Record<string, Function>,
    // The viewer is privileged: /__api is ungated, and there is no --trust for it
    // (Specs/Typebulb-CLI-Agent-Viewer.md).
    trusted: true,
  ...
  // the viewer. Identity is the `agent` field, not a `.bulb.md` path (a viewer has none) —
  // see Specs/Typebulb-CLI-Agent-Viewer.md. `file` is a sentinel for `logs`/`stop` display only.
  ...
  // Hot reload (dev). In the repo the viewer's source sits beside `dist/`, so watch it, rebuild the
    ...
    cleanupWatcher = watchDir({
      ...
      onChange: async () => {
        ...
        try {
          ...
          console.log('Viewer rebuilt. Browser reloading...\n')
          reloadEmitter.emit('reload')
        } catch (e) {
          console.error('Viewer rebuild failed:', e instanceof Error ? e.message : e)
        } finally {
          building = false
        }
      },
