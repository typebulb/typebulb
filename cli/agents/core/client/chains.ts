// Same-name chain supersession for live ````bulb```` inline bulbs, in transcript order.
//
// A consecutive run of inline bulbs sharing a (non-empty) frontmatter `name` is one evolving artifact: the
// agent emitting a revision under the same name. Every member but the last is *superseded* — folded to
// an unmounted stub while only the newest stays live. The rule reduces to a next-sibling check: an inline bulb
// is superseded iff the next inline bulb in order carries the same name. A differently-named (or unnamed) inline bulb
...
//
// Pure and DOM-free so it unit-tests in node. See TB-Agent-Mirror-Inline.md, Iteration
// Invariant 4.
export function supersededFlags(names: (string | undefined)[]): boolean[] {
  return names.map((name, i) => !!name && names[i + 1] === name)
}

// The 1-based position of each inline bulb within its consecutive same-name run — its "attempt" number.
