...
// The mirror's CORE: it tails CC's on-disk JSONL transcript and renders it; it drives nothing.
// Native only: it mirrors the real ~/.claude transcripts your terminal CC writes.
// See TB-Agent-Mirror.md for the architecture. The feature modules (launcher, switcher) sit apart and
// depend on this only for the shared search helper + the project cwd — never the reverse.
...
interface State {
  ...
  // The model the LAST assistant turn actually resolved to (entry.message.model) — the live
  // watchdog's anchor (TB-Agent-Switcher.md L1). The switcher owns only rung 3 of CC's model
  // ladder; this is the only way the mirror can *see* what rung 1/5 resolved to, so an
  // Anthropic model riding the OpenRouter route can be caught the turn it lands. null until a
  // first assistant turn carries one (a brand-new session).
...
function applyEntry(entry: JsonlEntry) {
  ...
  if (entry.type === 'user') {
    ...
    if (entry.message?.model) s.latestModel = entry.message.model
...
export async function poll(cursor: number) {
  const s = state
  refreshActive()
  // latestModel rides the poll (not switchState) so the watchdog is live without the menu being
  // opened: the pill turns red the instant a desynced turn lands on disk (L1).
  return { events: s.buffer.slice(cursor), cursor: s.buffer.length, working: chainWorking(s) && sessionAlive(s.sessionId), latestModel: s.latestModel }
}
...
// highlighted client-side. Shared by searchSessions (here) and the launcher's searchBulbs.
