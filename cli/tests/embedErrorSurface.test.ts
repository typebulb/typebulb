...
/**
 * The embed forwards failures to the host only via `window` 'error' / 'unhandledrejection'
 ...
 * listener never sees, so the dep 404s, React never mounts, the embed collapses to zero height,
 * and nothing reaches the log (the globe.gl failure). The embed must also surface this third
 ...
 * guards the floor: the embed registers capture-phase error handling at all.
 */
const base = {
...
describe('embed surfaces module/resource load failures', () => {
  it('registers a capture-phase error listener in the embed srcdoc', () => {
    const html = renderHtml({ ...base, embedded: true })
