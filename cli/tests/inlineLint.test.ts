...
// An inline bulb is client-only, so renderBulb runs the same `typebulb/lint` browser ruleset `typebulb check`
...
// `typebulb logs claude` via the existing compile-error readback (TB-Agent-Mirror-Inline.md, Iteration).

const wrap = (code: string) =>
  `---\nformat: typebulb/v1\nname: T\n---\n\n**code.tsx**\n\n\`\`\`tsx\n${code}\n\`\`\`\n`

describe('renderBulb lints client code (the inline bulb path)', () => {
