...
- **Launch once, and share the printed link.** `npx typebulb foo.bulb.md` starts the server (the page opens where the agent mirror is; with no mirror page open, VS Code's terminal and agent shells print the link to share rather than opening a tab). The link stays good: a bulb keeps its port across runs, keyed to the filename, so re-share it after a rename.
...
- **Don't relaunch, and don't wrap it in the `timeout` command.** A relaunch replaces the running server (one per bulb file), reclaiming the same port so the open tab reloads itself — it costs the page's in-memory state and nothing else. The `timeout` command kills the server outright, and the racing relaunch is what spawns extra windows; your harness's own tool timeout does not kill it. When you must relaunch, relaunch plainly: a relaunch replaces on its own, so `stop` first is a wasted step (harmless — it closes the page, and the relaunch opens a fresh one).
- **When something has left two pages open, `stop` then relaunch is the reset.** `stop` closes the bulb's pages before its server goes, so it is the one gesture that ends every copy; the relaunch gives you exactly one page again. Nothing else you can type reaches a browser tab.
