...
To keep this skill on hand across sessions, run `npx typebulb skill` and copy its output into your skills folder (e.g. for Claude Code, `.claude/skills/typebulb/SKILL.md`) — only if the user asks. On Pi this is unnecessary: the CLI keeps `~/.pi/agent/skills/typebulb/` installed and current automatically.
...
- **Assets**: push carries the bulb's `assets/` folder — files upload to typebulb's asset host and the published bulb serves them with zero config (small files; caps apply, refusals say so). To self-host instead, set `"assets": "https://your-host/path/"` in `config.json`: push then verifies each file exists there (misses refuse; `--force` publishes anyway). Viewers are redirected to the asset host; a local file shadows its hosted copy, and display needs no CORS (only canvas/`fetch()` pixel-reading does).
